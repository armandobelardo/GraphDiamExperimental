...
namespace Diameter {
  ...
  // TODO(iamabel): potentially clean up the below code and format like rest.
  ...
  // BFS is inlined to avoid time delay of a function call, this matches the
  // format of the paper's solution from @kawatea
  int GetPaperMyBFSDiam(const vector <vector<int> > &adjlist) {
    ...
    // Compute the diameter lower bound by the double sweep algorithm
    {
        for (int i = 0; i < num_double_sweep; i++) {
            ...
            source = lastInLevel;
            {
              visited = new bool[radjlist.size()];
              visited[source] = true;
              ...
              while (!lineup.empty()) {
                ...
                for (int neighbor : radjlist[curr]) {
                  if (!visited[neighbor]) {
                    visited[neighbor] = true;
                    lineup.push_back(neighbor);
                  }
    ...
    // Examine every vertex
    vector <int> ecc(V, V), queue(V), dist(V, -1);
    int qs, qt;
    {
        for (int i = 0; i < V; i++) {
            int u = order[i].second;
